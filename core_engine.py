...
def get_context(text:str,name,k):
    docs = get_or_init_doc_storage(name).similarity_search(text, k=k)
...
def answer_statue_related_questions(name,query_related_context,query,llm):
    QandA_prompt=PromptTemplate(
        input_variables=['name','query','query_related_context'],
